...
if [[ $VIDEO_ONLY == true ]]; then
    VIDEO_ONLY_OPTIONS="-map 0 -map -0:a "
else
    VIDEO_ONLY_OPTIONS=""
fi
ffmpeg  -i rtmp://127.0.0.1:$PORT_RTMP/live/stream   $VIDEO_ONLY_OPTIONS -codec copy -bsf:v h264_mp4toannexb -f rtsp  rtsp://127.0.0.1:$PORT_RTSP/rtsp/stream &
status=$?
if [ $status -ne 0 ]; then
  echo "Failed to start rtsp: $status"
  exit $status
